...
// ── Round Generator constants ────────────────────────────────────────────────
const GEN_RESTAURANTS = ["Pen Yen","Main Barn","Hay Barn","Mill + Toilets"];
const GEN_SINGLE_PICK = ["Glasshouse","Barwell"];       // pick one
const GEN_GROUP_A = ["Boathouse","Gym","Sauna & Steam Room"]; // always together
const GEN_GROUP_B = ["Gate House","Check-out House"];        // always together
const GEN_GROUP_C = ["Workshop","Flowers","Soho Home"];      // always together

// Generate a balanced rota for selected staff on a given day
function generateDayRota(staffList) {
  ...
  // 1. Restaurants — one per person (cycle if more staff than restaurants)
  staffList.forEach((staff, i) => {
    const restaurant = GEN_RESTAURANTS[i % GEN_RESTAURANTS.length];
    entries.push({
      id: uid(), type:"task",
      title: restaurant,
      location: restaurant,
      assigneeId: staff.id,
      priority: "medium",
      tasks: ["Clean & sanitise all surfaces","Vacuum/sweep floors","Empty bins","Check & restock supplies","Report any damages"],
    });
  });
  
  // 2. Single pick (Glasshouse or Barwell) — assign to first available
  const singlePickLoc = GEN_SINGLE_PICK[Math.floor(Math.random() * GEN_SINGLE_PICK.length)];
  entries.push({
    id: uid(), type:"task",
    title: singlePickLoc,
    location: singlePickLoc,
    assigneeId: staffList[0 % n].id,
    priority: "medium",
    tasks: ["Full clean","Vacuum/mop","Empty bins","Sanitise surfaces"],
  });
  
  // 3. Group A: Boathouse + Gym + Sauna — assign to person index 1 (or 0)
  const assignA = staffList[Math.min(1, n-1)].id;
  GEN_GROUP_A.forEach(loc => {
    entries.push({
      id: uid(), type:"task",
      title: loc, location: loc,
      assigneeId: assignA,
      priority: "medium",
      tasks: ["Clean & sanitise","Mop/vacuum floors","Empty bins","Check equipment","Restock towels/supplies"],
    });
  });
  
  // 4. Group B: Gate House + Check-out House — assign to person index 2 (or last)
  const assignB = staffList[Math.min(2, n-1)].id;
  GEN_GROUP_B.forEach(loc => {
    entries.push({
      id: uid(), type:"task",
      title: loc, location: loc,
      assigneeId: assignB,
  ...
  // 5. Group C: Workshop + Flowers + Soho Home — assign to person index 3 (or last)
  const assignC = staffList[Math.min(3, n-1)].id;
  GEN_GROUP_C.forEach(loc => {
    entries.push({
      id: uid(), type:"task",
      title: loc, location: loc,
      assigneeId: assignC,
...
function RotaGeneratorModal({allUsers,onGenerate,onClose}){
  ...
  const [selected,setSelected] = useState(
    Object.fromEntries(cleaners.map(u=>[u.id,true]))
  );
  const [singlePick,setSinglePick] = useState(GEN_SINGLE_PICK[0]);
  const toggle = id => setSelected(s=>({...s,[id]:!s[id]}));
  
  const selectedStaff = cleaners.filter(u=>selected[u.id]);
  
  const handleGenerate = () => {
    if (!selectedStaff.length){alert("Select at least one cleaner");return;}
    const entries = generateDayRota(selectedStaff, singlePick);
    // Override single pick with user selection
    const fixedEntries = entries.map(e=>
      GEN_SINGLE_PICK.includes(e.location)
        ? {...e, title:singlePick, location:singlePick}
        : e
    );
    onGenerate(fixedEntries);
    onClose();
  };
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#000000b0",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(8px)"}}>
      <div style={{background:"#111128",border:"1px solid #252540",borderRadius:20,width:"100%",maxWidth:520,padding:"28px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <div style={{marginBottom:18}}>
          <div style={{fontSize:11,color:"#555",textTransform:"uppercase",letterSpacing:1,fontWeight:700,marginBottom:10}}>
            Who is working today? ({selectedStaff.length} selected)
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
            {cleaners.map(u=>(
              <button key={u.id} onClick={()=>toggle(u.id)}
                style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:10,
                  background:selected[u.id]?"#4ade8022":"#0a0a1a",
                  border:`1px solid ${selected[u.id]?"#4ade80":"#252540"}`,
                  cursor:"pointer",textAlign:"left"}}>
                <div style={{width:22,height:22,borderRadius:"50%",flexShrink:0,
                  background:selected[u.id]?"#4ade80":"#252540",
                  border:`2px solid ${selected[u.id]?"#4ade80":"#444"}`,
                  display:"flex",alignItems:"center",justifyContent:"center",fontSize:12}}>
                  {selected[u.id]&&"✓"}
                </div>
                <div>
                  <div style={{fontSize:12,fontWeight:600,color:selected[u.id]?"#fff":"#666"}}>{u.name.split(" ")[0]}</div>
                  <div style={{fontSize:9,color:"#555"}}>{u.name.split(" ").slice(1).join(" ")}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        {/* Single pick location */}
        <div style={{marginBottom:20}}>
          <div style={{fontSize:11,color:"#555",textTransform:"uppercase",letterSpacing:1,fontWeight:700,marginBottom:8}}>
            Single location (choose one)
          </div>
          <div style={{display:"flex",gap:8}}>
            {GEN_SINGLE_PICK.map(loc=>(
              <button key={loc} onClick={()=>setSinglePick(loc)}
                style={{flex:1,padding:"10px",borderRadius:10,
                  background:singlePick===loc?"#d4a84322":"transparent",
                  border:`1px solid ${singlePick===loc?"#d4a843":"#252540"}`,
                  color:singlePick===loc?"#d4a843":"#555",fontWeight:700,fontSize:12,
        ...
        {selectedStaff.length>0&&(
          <div style={{background:"#0a0a1a",borderRadius:12,padding:"14px",marginBottom:18,fontSize:11,color:"#888",lineHeight:2}}>
            <div style={{fontWeight:700,color:"#aaa",marginBottom:6}}>Preview assignment:</div>
            {selectedStaff.map((s,i)=>(
              <div key={s.id} style={{display:"flex",gap:8,alignItems:"baseline"}}>
                <span style={{color:"#4ade80",fontWeight:700,minWidth:90,flexShrink:0}}>{s.name.split(" ")[0]}</span>
                <span style={{color:"#666"}}>
                  {GEN_RESTAURANTS[i%GEN_RESTAURANTS.length]}
                  {i===0?`, ${singlePick}`:""}
                  {i===Math.min(1,selectedStaff.length-1)?`, ${GEN_GROUP_A.join(", ")}`:""}
                  {i===Math.min(2,selectedStaff.length-1)?`, ${GEN_GROUP_B.join(", ")}`:""}
                  {i===Math.min(3,selectedStaff.length-1)?`, ${GEN_GROUP_C.join(", ")}`:""}
                </span>
              </div>
            ))}
          </div>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose}
            style={{flex:1,padding:"12px",background:"transparent",border:"1px solid #252540",borderRadius:12,color:"#555",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
            Cancel
          </button>
          <button onClick={handleGenerate}
            style={{flex:2,padding:"12px",background:"#d4a843",border:"none",borderRadius:12,color:"#000",fontWeight:800,fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
            ✨ Generate & Apply
          </button>
        </div>
      </div>
...
export default function AdminPanel(){
  ...
  return(
    ...
    <>
    <div style={{paddingBottom:40}}>eRef } from "react";
